...
// WebSocket con el servidor C++
const socket = new WebSocket("ws://localhost:5000");

// Guardamos la última posición recibida de cada enemigo
...
socket.onmessage = (event) => {
  ...
  // Guardar posición recibida desde C++
  enemyPositions[car.id] = {
    x: car.x,
    y: car.y
  };
...
window.onload = function () {
  ...
  function startGame() {
    ...
    // Evita que el chequeo de spawn de power-ups se ejecute en
    // cada frame (60 veces/seg). Sin esto, con SHIELD_SPAWN_CHANCE
    // = 0.6 se generaban decenas de power-ups apilados por segundo.
    let lastPowerupSpawnCheck = 0;
    const POWERUP_SPAWN_CHECK_INTERVAL_MS = 600;
    ...
    app.ticker.add(() => {
      ...
      if (gameStarted && !lost) {
        ...
        // Crear y actualizar enemigos según las posiciones recibidas del servidor
        for (const id in enemyPositions) {

          const position = enemyPositions[id];

          // Buscar si este carro ya existe
          let enemyCar = null;

          for (let i = 0; i < enemyCars.length; i++) {
            if (enemyCars[i].serverId == id) {
              enemyCar = enemyCars[i];
              break;
            }
          }
          ...
          if (enemyCar === null) {
            ...
            console.log("Enemigo creado desde servidor:", id);
          }

          // La posición viene directamente del servidor
          enemyCar.sprite.x = position.x;
          enemyCar.sprite.y = position.y;
        }

        // Chance to spawn power-ups (only if player has evaded enough cars,
        // NO power-up is currently active on the player, AND enough time
        // has passed since the last spawn attempt).
        const anyPowerUpActive = shieldActive || slowmoActive || slowmoTransitioning;

        if (
          carsEvadedCount >= GameConfig.POWERUP_START_THRESHOLD &&
          !anyPowerUpActive &&
          (now - lastPowerupSpawnCheck) >= POWERUP_SPAWN_CHECK_INTERVAL_MS
        ) {
          lastPowerupSpawnCheck = now;

          // Spawn shield (only if position is clear of enemies and
          // there isn't already one sitting uncollected on the road)
          if (shields.length === 0 && Math.random() < GameConfig.SHIELD_SPAWN_CHANCE) {
          ...
          // Spawn slow-mo (only if position is clear of enemies and
          // there isn't already one sitting uncollected on the road)
          if (slowmos.length === 0 && Math.random() < GameConfig.SLOWMO_SPAWN_CHANCE) {
            const randomLane = randomBetween(0, (scenario.lanes * 2) - 1);
        ...
        for (let i = 0; i < enemyCars.length; i++) {
          ...
          // La posición ya fue actualizada por el servidor C++
          if ((enemyBounds.y - car.height) < windowHeight) {
            // Actualizar límites después de recibir la posición
